...
},0);},cut:function(){setTimeout(function(){a.execute();},0);},cancel:function(){setTimeout(function(){a.execute();},0);}});return a;},setErrorMesssage:function(h){$("div.errorMessage",a.parent()).remove();
if(h.error){if(a.data("pos")==="top"){a.parent().prepend($("<div></div>").addClass("errorMessage").text(h.errorMessage));}else{a.parent().append($("<div></div>").addClass("errorMessage").text(h.errorMessage));
...
a.chkMinNum=d.chkMinNum;a.chkFormat=d.chkFormat;a.chkRegexp=d.chkRegexp;a.execute=d.execute;a.getError=d.getError;a.setValidation=d.setValidation;a.setValidation();
if(d.defaultErrorDisp){a.bind("inputcheck",function(i,h){d.setErrorMesssage(h);});}return a;}});(function(a){a(document).ready(function(){a('[data-role*="validation"]').each(function(){a(this).validate();
